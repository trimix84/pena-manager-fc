...
export default async function ConvocatoriaPage() {
  ...
  const confirmed =
    signups?.filter((s) => s.status === "going") ?? [];

  const reserves =
    signups?.filter((s) => s.status === "reserve") ?? [];

  const doubtful =
    signups?.filter((s) => s.status === "doubtful") ?? [];
          ...
          <div className="mt-3 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              ...
              <p className="mt-2 text-slate-300">
                {match?.format} · Máximo {match?.max_players} jugadores
              </p>
